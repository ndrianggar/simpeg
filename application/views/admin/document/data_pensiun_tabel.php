	...
	<script>
		$(document).ready(function(){
			var tabel = $('#tabel-data').DataTable({
				...
				"bAutoWidth": 	true,
				"bSort": 		false,
				"sAjaxSource": 	'<?php echo base_url(); ?>pensiun/data',
				"aoColumns":	[
									{ "mData"	: "no"},
									{ "mData"	: "surat"},
									{ "mData"	: "tanggal"},
			...
			$('#btn-tambah').click(function() {
				...
				// $('#kode_pegawai').focus();
				// $('#kode_pegawai').select();
				var url = "<?=base_url()?>pensiun/form_tambah/";
				...
				function(isConfirm){
					if (isConfirm) {
						$.ajax({
							type: 		'ajax',
							method: 	'post',
							url: 		'<?=base_url()?>'+'pensiun/hapus/' + kode,
							async: 		true,
							dataType: 	'json',
			...
			$('#tabel-body').on('click', '#btn-ubah', function(){
				var kode 	= $(this).data('kode');
				var url = "<?=base_url()?>pensiun/form_edit/" + kode;
				window.location.href = url;
			...
			$('#tabel-body').on('click', '#btn-pdf', function(){
				var kode 	= $(this).data('kode');
				var url = "<?=base_url()?>pensiun/cetak_pdf/" + kode;
