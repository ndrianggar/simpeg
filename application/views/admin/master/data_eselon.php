				...
				<div class="row">
					<div class="col-md-12 col-sm-12 col-xs-12">
						...
						<div class="modal fade" id="modal-tambah">
							<form id="form-tambah" data-parsley-validate method="POST" action="<?=base_url()?>eselon/tambah/">
										...
										<div class="modal-body">
											<div class="form-horizontal">
												<div class="form-group">
													<label class="col-md-4 col-sm-4 col-xs-12 control-label">Nama Eselon</label>
													<div class="col-md-8 col-sm-8 col-xs-12">
														<input id="tambah-kode" name="tambah-kode" type="hidden">
														<input id="tambah-nama" name="tambah-nama" class="form-control" placeholder="Nama Eselon" required="required">
													</div>
												</div>
											</div>
										</div>
	...
	<script>
		$(document).ready(function(){
			var tabel = $('#tabel-data').dataTable({
				...
				"sAjaxSource": 	'<?php echo base_url(); ?>eselon/data',
				"aoColumns":	[
									{ "mData"	: "no"},
									{ "mData"	: "nama"},
									{ "mData"	: "action"}
			...
			$('#btn-tambah').click(function(){
				$('#form-tambah').attr('action','<?=base_url()?>eselon/tambah');
			...
			$('#tabel-body').on('click', '#btn-ubah', function(){
				...
				$('#form-tambah').attr('action','<?=base_url()?>eselon/edit');
				...
				function(isConfirm){
					if (isConfirm) {
						$.ajax({
							type: 		'ajax',
							method: 	'post',
							url: 		'<?=base_url()?>'+'eselon/hapus/' + kode,
							async: 		true,
							dataType: 	'json',
							success: 	function(response){
								if(response==true){
