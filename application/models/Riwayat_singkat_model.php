<?php
class Riwayat_singkat_model extends CI_Model {
	...
	public function cari_cetak($kode) {
		$QuerySaya 		= $this->db->query(
						...
						E.nama_kecamatan,F.nama_kota,G.nama_propinsi, H.*,I.*,K.*,L.*,
						...
						LEFT JOIN t_riwayat_jabatan K ON A.id_pegawai=K.id_pegawai)
						LEFT JOIN t_jabatan L         ON K.kd_jabatan=L.kd_jabatan)
	...
	public function cari_pasangan($kode) {
		$QuerySaya 		= $this->db->query(
						  "SELECT A.*, B.*,C.*
							...
							WHERE B.sts_pegawai<>'1' AND A.kd_pegawai='$kode' AND A.sts_keluarga<>'1'"
						);
		return $QuerySaya->result();
	}
