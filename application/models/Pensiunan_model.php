...
class Pensiunan_model extends CI_Model {
	...
	public function cari_semua() {
		$QuerySaya 		= $this->db->query(
						  "SELECT A.*,B.nm_agama,C.nm_jenis,D.nama_kelurahan,
							...
							WHERE A.sts_pegawai<>'1' AND A.sts_pensiun='1'
	...
	public function cari_data_pensiunan($id) {
		$QuerySaya 		= $this->db->query(
	...
	public function tambah( $id, $tgl, $sk, $keterangan ) {
		$QuerySaya 		= $this->db->query(
						  "UPDATE t_pegawai SET
						  tgl_pensiun = '$tgl', sk_pensiun= '$sk', ket_pensiun='$keterangan', sts_pensiun='1'
						  WHERE id_pegawai = '$id';"
						);
		if ($QuerySaya) {
			return TRUE;
