...
<body class="nav-md">
	...
	<script>
		$(document).ready(function(){
						...
						$.ajax({
							...
							success: 	function(response){
								if(response==true){
									...
									tabel.api().ajax.reload();
									swal("PILIH!", "Pilih Admin Sukses.", "success");
									tabel_pegawai.api().ajax.reload();
								}else{
			...
			$('#tabel-pegawai').on('click', '#btn-pilih-pimpinan', function(){
				var kode 	= $(this).data('id');
				var nama 	= $(this).data('nama');
				swal({
					title: "Apakah anda yakin?",
				...
				function(isConfirm){
					if (isConfirm) {
						$.ajax({
							type: 		'ajax',
							method: 	'post',
							url: 		'<?=base_url()?>'+'pengurus/pilih_pimpinan/' + kode,
							async: 		true,
							dataType: 	'json',
							success: 	function(response){
								if(response==true){
									$('#modal-pegawai').modal('hide');
									tabel.api().ajax.reload();
									swal("PILIH!", "Pilih Pimpinan Sukses.", "success");
									tabel_pegawai.api().ajax.reload();
								}else{
									swal("ERROR", "Pilih data gagal.", "error");
								}
